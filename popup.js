...
var getJiraIssue = function (url) {
  ...
  return searchParams.selectedIssue
    ? searchParams.selectedIssue
    : url.pathname.replace("/browse/", "");
};
...
var setJiraIssueCodeFromAPI = function (url) {
  ...
  fetch(url.origin + "/rest/graphql/1/", {
    ...
    .then(function (json) {
      if (json) {
        var summary = json.data.issue.fields.find(
          (o) => o.key === "summary"
        ).content;
...
var setTrelloIssueCode = function (url) {
  chrome.storage.sync.get("trelloConfig", function (data) {
    ...
    if (!issuePattern) {
      alert.innerHTML =
        "Configure o padrão de issue nas configurações da extensão";
      return;
    }
    ...
    chrome.scripting.executeScript(
      ...
      (results) => {
        ...
        navigator.clipboard.writeText(issueDesc.value).then(
          function () {
            alert.innerHTML = "Copiado!";
          },
          function (err) {
